...
static std::string llama_token_to_text(const struct llama_context *ctx,
                                       llama_token token) {
...
static std::vector<llama_vocab::id> llama_tokenize_internal(
    const llama_vocab &vocab, const std::string &raw_text, bool bos,
    bool escape);
...
struct llm_tokenizer_bpe {
  llm_tokenizer_bpe(const llama_vocab &vocab, bool g2ws) : vocab(vocab) {
    flag_g2ws = g2ws;
  }
  ...
  for (size_t i = 0; i < candidates->size; ++i) {
    ...
    if (id == eos) {
      ...
      candidates_decoded.push_back(
          decode_utf8(text.c_str(), grammar->partial_utf8));
    ...
    if (!quantize) {
      ...
      if (name == tn(LLM_TENSOR_OUTPUT, "weight")) {
        ...
        if (ftype == LLAMA_FTYPE_MOSTLY_Q2_K)
          ...
          new_type = GGML_TYPE_Q6_K;
        else if (ftype == LLAMA_FTYPE_MOSTLY_Q4_K_S && i_feed_forward_w2 < 4)
          new_type = GGML_TYPE_Q5_K;
        ++i_feed_forward_w2;
      ...
      if (new_type == GGML_TYPE_Q2_K || new_type == GGML_TYPE_Q3_K ||
        ...
        if (nx % QK_K != 0 || ny % QK_K != 0) {
          LLAMA_LOG_INFO(
              "\n\nTensor sizes %d x %d are not divisible by %d, required for "
              "k-quants.\n",
              nx, ny, QK_K);
...
#define LLAMA_METAL_CHECK_BUF(result)                        \
  if (!(result)) {                                           \
    ...
    LLAMA_METAL_CHECK_BUF(ggml_metal_add_buffer(
        ctx->ctx_metal, "kv", ctx->kv_self.buf.data, ctx->kv_self.buf.size, 0));
...
int llama_model_type(const struct llama_model *model, char *buf,
                     size_t buf_size) {
...
int llama_token_to_str(const struct llama_context *ctx, llama_token token,
                       char *buf, int length) {
  return llama_token_to_str_with_model(&ctx->model, token, buf, length);
}

// does not write null-terminator to str
int llama_token_to_str_with_model(const struct llama_model *model,
...
void llama_print_timings(struct llama_context *ctx) {
  ...
  LLAMA_LOG_INFO("%s:        load time = %8.2f ms\n", __func__,
                 timings.t_load_ms);
  LLAMA_LOG_INFO(
      "%s:      sample time = %8.2f ms / %5d runs   (%8.2f ms per token, %8.2f "
      "tokens per second)\n",
  ...
  LLAMA_LOG_INFO(
      "%s: prompt eval time = %8.2f ms / %5d tokens (%8.2f ms per token, %8.2f "
      "tokens per second)\n",
  ...
  LLAMA_LOG_INFO(
      "%s:        eval time = %8.2f ms / %5d runs   (%8.2f ms per token, %8.2f "
      "tokens per second)\n",
