...
static T checked_mul(T a, T b) {
  T ret = a * b;
  if (a != 0 && ret / a != b) {
    throw format("overflow multiplying %llu * %llu", (unsigned long long)a,
...
struct llama_file_loader {
  ...
  void read_magic() {
    ...
    throw format(
        "unknown (magic, version) combination: %08x, %08x; is this really a "
        "GGML file?",
        magic, version);
  }
  void read_hparams() {
  ...
  void read_tensor_metadata(size_t file_idx,
    ...
    while (file.tell() < file.size) {
      ...
      if (n_dims < 1 || n_dims > 2) {
        throw format("llama.cpp: tensor '%s' should not be %u-dimensional",
                     name.c_str(), n_dims);
      }
      switch (shard.type) {
        case GGML_TYPE_F32:
...
struct llama_model_loader {
  ...
  struct ggml_tensor *get_tensor(const std::string &name,
    ...
    auto it = tensors_map.name_to_idx.find(name);
    if (it == tensors_map.name_to_idx.end()) {
      throw format("llama.cpp: tensor '%s' is missing from model",
                   name.c_str());
    }
    llama_load_tensor &lt = tensors_map.tensors.at(it->second);
    if (lt.ne != ne) {
      throw format(
          "llama.cpp: tensor '%s' has wrong shape; expected %s, got %s",
...
static void llama_model_load_internal(const std::string &fname,
                                      llama_context &lctx, int n_ctx,
                                      int n_gpu_layers, ggml_type memory_type,
...
#define LLAMA_BACKEND_OFFLOAD GGML_BACKEND_CPU
    ...
    for (uint32_t i = 0; i < n_layer; ++i) {
      ...
      if (backend == GGML_BACKEND_CUDA) {
        vram_total += ggml_nbytes(layer.attention_norm) +
                      ...
                      ggml_nbytes(layer.attention_norm) +
  ...
  // print memory requirements
  {
    ...
    const size_t mem_required =
        ctx_size + mmapped_size - vram_total +  // weights in VRAM not in memory
...
#ifdef GGML_USE_CUBLAS
  {
    ...
    for (llama_load_tensor &lt : ml->tensors_map.tensors) {
      ...
      ggml_cuda_load_data(fname.c_str(), lt.ggml_tensor,
                          lt.shards.at(0).file_off);
      done_size += lt.size;
    }
  }
...
static bool llama_model_load(const std::string &fname, llama_context &lctx,
                             int n_ctx, int n_gpu_layers, ggml_type memory_type,
                             bool use_mmap, bool use_mlock, bool vocab_only,
                             llama_progress_callback progress_callback,
                             void *progress_callback_user_data) {
  try {
    llama_model_load_internal(fname, lctx, n_ctx, n_gpu_layers, memory_type,
...
static bool llama_eval_internal(llama_context &lctx, const llama_token *tokens,
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // self-attention
    {
      ...
      struct ggml_tensor *Qcur = ggml_rope_inplace(
          ctx0,
          ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, model.layers[il].wq, cur),
                          n_embd / n_head, n_head, N),
          n_past, n_rot, 0);
      struct ggml_tensor *Kcur = ggml_rope_inplace(
          ctx0,
          ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, model.layers[il].wk, cur),
                          n_embd / n_head, n_head, N),
          n_past, n_rot, 0);
      ...
      // store key and value to memory
      {
        ...
        struct ggml_tensor *Vcur = ggml_transpose(
            ctx0,
            ggml_reshape_2d(ctx0, ggml_mul_mat(ctx0, model.layers[il].wv, cur),
                            n_embd, N));
  ...
  // extract logits
  {
    ...
    if (lctx.logits_all) {
      ...
      memcpy(logits_out.data(), (float *)ggml_get_data(inpL),
...
static void llama_model_quantize_internal(const std::string &fname_inp,
                                          const std::string &fname_out,
                                          enum llama_ftype ftype, int nthread) {
  ...
  llama_file_saver file_saver(fname_out.c_str(),
                              model_loader->file_loaders.at(0).get(), ftype);
...
struct llama_context *llama_init_from_file(const char *path_model,
  ...
  if (!llama_model_load(path_model, *ctx, params.n_ctx, params.n_gpu_layers,
                        memory_type, params.use_mmap, params.use_mlock,
